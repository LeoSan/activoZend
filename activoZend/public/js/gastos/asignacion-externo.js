		...
		init : function() {
			...
			AsignacionExterno.externo.find( "#cbjuicio" ).on({
				change : function( e ) {
					...
					var url = baseUrl + "/gastos/asignacion/get-info-juicio";
					$.ajax({
						type: "POST",
						url: url,
						data: { 
...
//					var url = "http://quasar.pendulum.com.mx:59001/el/aplicacion/creditos/credito?dmacct=" + credito + "&user=" + $("#usuariopm").val();
			...
			$("#btnBuscarCredito").on('click',function(e){
				...
				$.ajax({
					url: baseUrl + '/gastos/asignacion/find-credito',
					type: 'POST',
		...
		guardarAltaIndividual : function(e, externo) {
			...
			var urlAjax = baseUrl + '/gastos/asignacion/addcreditoasigna';
			if (externo) {
				var urlAjax = baseUrl + '/gastos/asignacion/addcreditoasigna-externo';
			}			
